...
export async function email(message, env, ctx) {
	...
	try {
		...
		const { feishuBotStatus, feishuChatId, feishuOpenId, feishuReceiveType } = feishuSetting;
		console.log(`[Feishu] 邮件接收流程 - 状态:${feishuBotStatus}, 接收类型:${feishuReceiveType === 1 ? '个人' : '群聊'}, 目标ID:${feishuReceiveType === 1 ? (feishuOpenId ? feishuOpenId.substring(0,8)+'...' : '空') : (feishuChatId ? feishuChatId.substring(0,8)+'...' : '空')}`);
		
		// 根据接收类型判断是否有目标ID
		const hasFeishuTarget = feishuReceiveType === 1 ? feishuOpenId : feishuChatId;
		if (feishuBotStatus === 0 && hasFeishuTarget) {
			try {
